...
func setupTemplate(c *config) {
	f := template.FuncMap{
		...
		"TypeKind":            func(t schema.Type) string { return cases.Title(language.English).String(t.TypeKind().String()) },
		"Title":               func(name string) string { return cases.Title(language.English).String(name) },
